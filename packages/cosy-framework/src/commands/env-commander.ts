...
`
    ...
    .action(async (options: { simple?: boolean; json?: boolean }) => {
...
function displayDetailedInfo(envInfo: any): void {
  ...
  memoryTable.push(
    ...
    [
      '系统已用',
      `${formatBytes(envInfo.memory.system.used)} (${envInfo.memory.system.usagePercent}%)`,
    ],
    ['进程内存', formatBytes(envInfo.memory.process.rss)],
    [
      '堆内存',
      `${formatBytes(envInfo.memory.process.heapUsed)} / ${formatBytes(envInfo.memory.process.heapTotal)}`,
    ]
  );
...
function displaySimpleInfo(envInfo: any): void {
  ...
  table.push(
    ...
    [
      '💾 内存使用',
      `${envInfo.memory.system.usagePercent}% (${formatBytes(envInfo.memory.system.used)}/${formatBytes(envInfo.memory.system.total)})`,
    ],
    ['📁 工作目录', envInfo.framework.workingDirectory]
  );
